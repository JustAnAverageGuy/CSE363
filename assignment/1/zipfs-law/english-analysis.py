...
root = BASE_PATH / Path(ENGLISH_PATH)
processed_english_path = BASE_PATH / Path("./english_freq.json")
...
def to_words(path):
    ...
    if text is None:
        return []
    words = [i.lower() for i in re.findall(TERM_PATTERN, text)]
    return words
...
def to_words_inplace(path):
    ...
    if text is None:
        return
    for word in re.findall(TERM_PATTERN, text):
        wc[word.lower()] += 1
